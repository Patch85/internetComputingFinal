<?php
    ...
    // If update has been submitted, update cart contents
    if (!empty($_POST)) {
        foreach ($_POST as $name => $value) {
            $theName = $name;
            $prodName = array_column($_SESSION['cart'], 'productName');
            $found_key = array_search($theName, $prodName);
            $_SESSION['cart'][$found_key]['quantity'] = $value;
        }
...
<body>
    ...
    <div class="jumbotron jumbotron-fluid musicJumbotron">
        <div class="container">
            <h1 class="jumboHeading">Shopping Cart</h1>
        </div>
    </div>
